...
const SCALE_TYPES = ["ColorBrew-RdYlGn", "ColorBrew-RdYlGn-old", "ColorBrew-PiYG", "ColorBrew-BrBG"]
...
export const INITIAL_STATE: SettingsState = {
  ...
  actionsDone: [],
  steps: ["rating", "sleep", "emotions", "tags", "message"]
}

type Value = {
  settings: SettingsState
  setSettings: (settings: SettingsState | ((settings: SettingsState) => SettingsState)) => void
  resetSettings: () => void
...
function SettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const removeActionDone = useCallback(
    (actionTitle: IAction["title"]) => {
      setSettings(settings => ({
        ...settings,
        actionsDone: settings.actionsDone.filter(action => action.title !== actionTitle)
      }))
    },
    [settings.actionsDone]
  )
