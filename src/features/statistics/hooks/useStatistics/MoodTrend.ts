...
export const getMoodTrendData = (items: LogItem[]): MoodTrendData => {
  ...
  for (let i = SCALE_RANGE / 2; i < SCALE_RANGE; i++) {
    ...
    if (_items.length > 0) {
      ratingAverage =
        Math.floor((_items.reduce((acc, item) => acc + item.value, 0) / _items.length) * 100) / 100
    }
  ...
  const avgPeriod1 =
    ratingsPeriode1.reduce((acc, item) => acc + item.value, 0) / ratingsPeriode1.length
  const avgPeriod2 =
    ratingsPeriode2.reduce((acc, item) => acc + item.value, 0) / ratingsPeriode2.length
